import {
	...
	type StreamOptions,
} from "@pipyter/pigent-ai";
import { getApiProvider } from "@pipyter/pigent-ai/compat";
import type { ModelConfig, SettingsModelDefinition, SettingsModelOverride, SettingsProviderConfig } from "./model-config.ts";
import {
...
export type AuthStatus = {
	configured: boolean;
	source?: "stored" | "runtime" | "environment" | "fallback" | "settings_key" | "settings_command";
	label?: string;
};
...
function mergeCompat(
	...
	override: Model<Api>["compat"] | SettingsModelOverride["compat"],
): Model<Api>["compat"] {
...
function applyModelOverride(model: Model<Api>, override: SettingsModelOverride): Model<Api> {
	return {
		...model,
...
function modelFromJson(
	...
	providerConfig: SettingsProviderConfig,
...
function applyModelConfig(
	...
	config: SettingsProviderConfig | undefined,
): Model<Api>[] {
...
export function validateExtensionProvider(
	providerId: string,
	base: Provider | undefined,
	modelsConfig: SettingsProviderConfig | undefined,
	...
	applyExtension(providerId, applyModelConfig(providerId, base?.getModels() ?? [], modelsConfig), extension);
}

/** Compose built-in, settings, and extension layers without reading credentials. */
export function composeModelProvider(
	...
	// Settings modelOverrides are the topmost user-config layer: they apply once,
	// after custom-model upserts, extension model replacement, and OAuth projection.
	const getModels = () => {
		let models = applyExtension(
			providerId,
			applyModelConfig(providerId, base?.getModels() ?? [], config),
			currentExtension(),
		);
				...
				? async (context) => {
						...
						if (extension?.refreshModels) {
							...
							if (!context.signal?.aborted) {
								...
								applyExtension(providerId, applyModelConfig(providerId, base?.getModels() ?? [], config), {
									...extension,
									models: refreshed,
								});
...
export function configuredRequestAuthStatus(
	...
	if (isCommandConfigValue(value)) return { configured: true, source: "settings_command" };
	...
	return { configured: true, source: extension?.apiKey !== undefined ? "fallback" : "settings_key" };
}
