...
import * as builtinProviderCatalog from "@pipyter/pigent-ai/providers/all";
...
export interface CreateModelRuntimeOptions {
	...
	authPath?: string;
	/** settings.json may contain models.providers. Defaults to the agent directory. Set to null to disable. */
	settingsPath?: string | null;
	/** Catalog persistence is opt-in; the default store is in-memory. */
...
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const settingsPath =
			options.settingsPath === null ? undefined : (options.settingsPath ?? join(getAgentDir(), "settings.json"));
		const config = await ModelConfig.load({ settingsPath });
		const modelsStore = options.modelsStore ?? new InMemoryCodingAgentModelsStore();
	...
	async refresh(options: ModelsRefreshOptions = {}): Promise<ModelsRefreshResult> {
		this.config = await ModelConfig.load({ settingsPath: this.settingsPath });
