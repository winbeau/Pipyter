...
import type { ThinkingLevel } from "@pipyter/pigent-agent";
import type { Model } from "@pipyter/pigent-ai";
 ...
 * Pigent passes exact Pipyter config paths: the user config directory is
 * `${XDG_CONFIG_HOME:-~/.config}/pipyter/pigent` and model selection lives in
 * `settings.json`/`auth.json` only; the model catalog store is in-memory
 * (docs/plans/pigent-v0.1/09-user-install-model-config.md).
 */
