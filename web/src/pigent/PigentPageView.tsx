...
function EventView({ event, onOpenShell, reviewBeforeApply }: { event: PigentEvent; onOpenShell(id?: string): void; reviewBeforeApply: boolean }) {
  ...
  if (event.type === 'assistant.text' || event.type === 'assistant.thinking' || event.type === 'settled') {
    const text = String(event.payload?.text ?? event.payload?.summary ?? (event.type === 'settled' ? '会话已完成' : 'Pigent 正在分析…'))
...
export function PigentPageView() {
  ...
  const events = useMemo(() => Object.values(state.eventsById).sort((a, b) => a.event_id - b.event_id).slice(-300), [state.eventsById])
  ...
  return <div className={`pigent-page${state.detailOpen ? ' has-detail' : ''}`}>
    ...
    <main className="pigent-main-column">
      <PigentHeader session={session} mode={state.mode} pendingMode={state.pendingMode} onMode={(mode) => void actions.setMode(mode)} runActive={state.runActive} detailOpen={state.detailOpen} onDetail={() => actions.setDetailOpen(!state.detailOpen)} />
      ...
      <div className="pigent-composer-wrap"><Composer running={state.runActive} onSend={actions.send} /></div>
    </main>
