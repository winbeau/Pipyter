...
import { parseEnvelope, shellApi, shellStreamUrl } from './api'
...
export function ShellProvider({ children }: { children: ReactNode }) {
 ...
 const connect = useCallback((session: TerminalSession) => {
  ...
  const socket = new WebSocket(shellStreamUrl(session.id, cursors.current.get(session.id) ?? 0)); socket.binaryType = 'arraybuffer'; sockets.current.set(session.id, socket)
  ...
  socket.onmessage = (message) => {
   ...
   else if (envelope.type === 'exit') { cursors.current.set(session.id, envelope.cursor); void shellApi.list().then((sessions) => dispatch({ type: 'sessions', sessions: liveSessions(sessions) })) }
  }
 ...
 const refresh = useCallback(async () => { try { const sessions = liveSessions(await shellApi.list()); dispatch({ type: 'sessions', sessions }); sessions.forEach(connect); dispatch({ type: 'connection', state: sessions.length ? 'connecting' : 'connected' }) } catch (error) { dispatch({ type: 'connection', state: 'disconnected', error: error instanceof Error ? error.message : String(error) }) } }, [connect])
 useEffect(() => { void refresh(); const open = (event: Event) => { const id = (event as CustomEvent<{ sessionId?: string }>).detail?.sessionId; dispatch({ type: 'request', value: true }); if (id) { void shellApi.list().then((sessions) => { sessions = liveSessions(sessions); dispatch({ type: 'sessions', sessions }); const session = sessions.find((item) => item.id === id); if (session) { connect(session); dispatch({ type: 'select', id }) } }) } }; window.addEventListener('pipyter:open-shell', open); return () => { window.removeEventListener('pipyter:open-shell', open); sockets.current.forEach((socket) => socket.close()) } }, [connect, refresh])
 const create = useCallback(async () => { const session = await shellApi.create('.'); dispatch({ type: 'sessions', sessions: [...stateRef.current.sessions, session] }); dispatch({ type: 'select', id: session.id }); connect(session); dispatch({ type: 'request', value: true }); return session }, [connect])
 const actions = useMemo<ShellActions>(() => ({ refresh, create, select: (id) => { dispatch({ type: 'select', id }); const session = stateRef.current.sessions.find((item) => item.id === id); if (session) connect(session) }, send: (id, data) => { const socket = sockets.current.get(id); if (socket?.readyState === WebSocket.OPEN) socket.send(data) }, resize: (id, cols, rows) => { const socket = sockets.current.get(id); if (socket?.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ version: 1, type: 'resize', cols, rows })); else void shellApi.resize(id, cols, rows) }, clear: (id) => { const selected = id ?? stateRef.current.activeSessionId; if (selected) dispatch({ type: 'clear', id: selected }) }, close: async (id) => { const session = stateRef.current.sessions.find((item) => item.id === id); if (session?.status === 'running' && !window.confirm(`关闭正在运行的 Shell “${session.name}”？`)) return; sockets.current.get(id)?.close(); await shellApi.close(id); dispatch({ type: 'sessions', sessions: stateRef.current.sessions.filter((item) => item.id !== id) }) }, split: async () => { if (stateRef.current.panes.length >= 2) return; let other = stateRef.current.sessions.find((item) => item.id !== stateRef.current.activeSessionId); if (!other) other = await create(); dispatch({ type: 'panes', panes: [...stateRef.current.panes, { id: `pane-${Date.now()}`, sessionId: other.id }] }) }, setHeight: (height) => { const value = Math.max(160, Math.min(650, height)); localStorage.setItem(HEIGHT_KEY, String(value)); dispatch({ type: 'height', height: value }) }, setMaximized: (value) => dispatch({ type: 'maximize', value }), requestPanel: (id) => { dispatch({ type: 'request', value: true }); if (id) dispatch({ type: 'select', id }) }, acknowledgePanel: () => dispatch({ type: 'request', value: false }) }), [connect, create, refresh])
